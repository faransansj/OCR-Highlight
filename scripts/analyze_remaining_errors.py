...
from ocr.multi_ocr import MultiOCREngine
...
def analyze_remaining_errors():
    ...
    """
    print("\n" + "=" * 70)
    print("DETAILED ANALYSIS OF REMAINING 25 ERRORS (Ensemble)")
    ...
    for sample in val_data:
        ...
        if not os.path.exists(image_path):
            # Try prepending 'images/' if not found
            parts = image_path.split('/')
            if len(parts) >= 2:
                image_path = os.path.join(parts[0], parts[1], 'images', parts[2])
        ...
        for det in detections:
            ...
            if best_match:
                ...
                # Use ensemble_extract
                x, y, w, h = det['bbox']
                region = image[y:y+h, x:x+w]
                results = ocr_engine.ensemble_extract(region, color_hint=det['color'])
                
                pred_text = "".join([r.text for r in results])
                conf = np.mean([r.confidence for r in results]) * 100 if results else 0
